...
$db = mysqli_connect(DB_HOST,DB_USER,DB_PASS,'indra') or die("database is not connected");
...
function validateUpdateForm($form_data,$image_data){
    ...
    $response['status']=true;
       ...
       if($image_data['name']){
           ...
           if($type!='jpg' && $type!='jpeg' && $type!='png' && $type!='mp4' && $type!='mov'){
            $response['msg']="only jpg,jpeg,png,mp4,mov images are allowed";
            $response['status']=false;
